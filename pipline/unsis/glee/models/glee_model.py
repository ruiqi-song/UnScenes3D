...
Date         : 2024-04-15 14:40:37
FilePath     : /VLMs_glee/detectron2/projects/glee/models/glee_model.py
Description  : 
LastEditTime : 2024-04-18 14:57:01
...
def agg_lang_feat(features, mask, pool_type="average"):
    ...
    if pool_type == "average":
        ...
        for i in range(len(features)):
            pool_feat, _ = torch.max(
                features[i][mask[i]], 0)  # (L, C) -> (C, )
...
class GLEE_Model(nn.Module):
    ...
    def __init__(self, cfg, matcher, device, video_info, contras_mean):
        ...
        output_channels = [
            v for k, v in self.backbone._out_feature_channels.items()]
        ...
        if cfg.MODEL.TEXT.ARCH == 'clip_frozen':
            ...
            self.lang_projection = nn.Parameter(torch.rand(
                cfg.MODEL.LANGUAGE_BACKBONE.LANG_DIM, cfg.MODEL.DIM_PROJ))
            ...
            self.text_encoder = CLIPTextModel.from_pretrained(
                './weights/clip_vit_base_patch32')
            self.text_encoder_teacher = CLIPTextModel.from_pretrained(
                './weights/clip_vit_base_patch32')
            self.lang_encoder = None
        ...
        self.predictor = build_transformer_decoder(
            cfg, transformer_predictor_in_channels, lang_encoder=self.lang_encoder, mask_classification=True,)
        self.to(device)
        ...
        self.no_mask_tasks = ['obj365', 'obj365_clip', 'openimage',
                              'openimage_clip', 'vg', 'grit', 'bdd_det', 'bdd_track_box']

        # for visual prompt
        hidden_dim = 256
        self.max_spatial_len = [512, 512, 512, 512]
        self.mask_sptial_embed = nn.ParameterList(
            [nn.Parameter(torch.empty(hidden_dim, hidden_dim)) for x in range(4)])
    ...
    def forward(self, images, prompts, task, targets=None, batch_name_list=None, is_train=True, visual_prompt_type='scribble'):
        ...
        if self.text_encode_type == 'clip_frozen':
            if task not in ['grounding', 'rvos']:
                ...
                lang_feat_pool = agg_lang_feat(
                    token_x, tokenized['attention_mask'], pool_type="average")  # (bs, 768)
                extra['class_embeddings'] = lang_feat_pool
                dist_loss = (lang_feat_pool*0).sum()
                if self.early_fusion:  # early_fusion
                    gather_all_classtoken = token_x.flatten(
                        0, 1)[tokenized['attention_mask'].flatten(0, 1) > 0]
                    gather_all_classtoken = gather_all_classtoken.unsqueeze(
                        0).repeat(len(images), 1, 1)  # [bs,L,C]
                    gather_all_classtoken_mask = torch.ones_like(
                        gather_all_classtoken[:, :, 0]) > 0  # [bs,L]
                    early_semantic = {"hidden": gather_all_classtoken.float(
                    ), "masks": gather_all_classtoken_mask}
            ...
            if task not in ['grounding', 'rvos']:
                ...
                if self.early_fusion:  # class early_fusion
                    gather_all_classtoken = lang_feat_final.unsqueeze(
                        0).repeat(len(images), 1, 1)  # [bs,L,C]
                    gather_all_classtoken_mask = torch.ones_like(
                        gather_all_classtoken[:, :, 0]) > 0  # [bs,L]
                    early_semantic = {"hidden": gather_all_classtoken.float(
            ...
            if task not in ['grounding', 'rvos']:
                ...
                token_x_teacher = self.text_encoder_teacher(
                    *texts)['last_hidden_state']
                # if is_train:
                dist_loss = F.mse_loss(
                    token_x[valid_mask], token_x_teacher[valid_mask])
        ...
        if 'grounding' in prompts:
            ...
            if self.text_encode_type == 'clip_frozen' or self.text_encode_type == 'clip_teacher':
                ...
                tokens = {key: value.to(images.device)
                ...
                extra['grounding_tokens'] = token_x.permute(
                    1, 0, 2)  # [len,bz,C]

                non_zero_query_mask = tokens['attention_mask']
                lang_feat_pool = agg_lang_feat(
                    token_x, non_zero_query_mask, pool_type="average").unsqueeze(1)  # (bs, 1, 768)

                dist_loss = (lang_feat_pool*0).sum()

                # [bz,len]
                extra['grounding_nonzero_mask'] = ~non_zero_query_mask.bool()
                ...
                early_semantic = {"hidden": token_x.float(
        ...
        if 'spatial' in prompts:
            ...
            key_promptmasks = [m.unsqueeze(
                0) for m in prompts['spatial']]  # bz*[1,1,H,W]
            ...
            if is_train:
                if np.random.rand() > 0.6:
                    ...
                    if np.random.rand() > 0.8:  # box mode
                        ...
                        # samole a point, extend a  [H//20,W//20] rectangle mask
                        prompt_mode = 'point'
                        # Get a random pos point
                        non_zero_pos_points = [rand_sample(
                            (m.nonzero()[:, 1:]).t(), 1).t() for m in prompts['spatial']]
                        new_point2mask = []
                        ...
                        for point, pmask in zip(non_zero_pos_points, key_promptmasks):
                            ...
                            zeromask[:, :, point[0, 0]-point_h: point[0, 0]+point_h,
                                     point[0, 1]-point_w:point[0, 1]+point_w] = True
                    ...
                    for ori_mask, pmask in zip(prompts['spatial'], key_promptmasks):
                        ...
                        x1, y1, x2, y2 = masks_to_boxes(
                            pmask[0])[0].long().tolist()  # [xyxy]
            ...
            ref_feats, ref_masks = self.get_template(
        ...
        # bz = len(images)//2
        mask_features, _, multi_scale_features, zero_loss = self.pixel_decoder.forward_features(
            features, masks=None, early_fusion=early_semantic)
        if 'spatial' in prompts:
            ...
            ), self.max_spatial_len[-1]).t() for m in extra['spatial_query_pos_mask']]
            non_zero_pos_point = nn.utils.rnn.pad_sequence(
                non_zero_pos_point, padding_value=-1).permute(1, 0, 2)
            non_zero_pos_mask = (non_zero_pos_point.sum(dim=-1) < 0)
            spatial_query_pos = point_sample(mask_features, non_zero_pos_point.flip(
                dims=(2,)).type(mask_features.dtype), align_corners=True)  # [(N, C, P)
            spatial_query_pos = torch.stack([x[m].mean(dim=0, keepdim=True) for x, m in zip(
                spatial_query_pos.transpose(1, 2), ~non_zero_pos_mask)]).transpose(0, 1).nan_to_num()  # [1,bz,C]
            # Get mean neg spatial query
            non_zero_neg_point = [rand_sample((m.nonzero()[:, 1:]/divisor).t(
            ), self.max_spatial_len[-1]).t() for m in extra['spatial_query_neg_mask']]
            non_zero_neg_point = nn.utils.rnn.pad_sequence(
                non_zero_neg_point, padding_value=-1).permute(1, 0, 2)
            non_zero_neg_mask = (non_zero_neg_point.sum(dim=-1) < 0)
            spatial_query_neg = point_sample(mask_features, non_zero_neg_point.flip(
                dims=(2,)).type(mask_features.dtype), align_corners=True)
            spatial_query_neg = torch.stack([x[m].mean(dim=0, keepdim=True) for x, m in zip(
                spatial_query_neg.transpose(1, 2), ~non_zero_neg_mask)]).transpose(0, 1).nan_to_num()
            ...
            for i in range(len(multi_scale_features)):
                ...
                non_zero_query_point_pos = [rand_sample((m.nonzero()[:, 1:]/divisor).t(
                ), self.max_spatial_len[i]).t() for m in extra['spatial_query_pos_mask']]
                non_zero_query_point_neg = [rand_sample((m.nonzero()[:, 1:]/divisor).t(
                ), self.max_spatial_len[i]).t() for m in extra['spatial_query_neg_mask']]
                non_zero_query_point = [torch.cat([x, y], dim=0) for x, y in zip(
                    non_zero_query_point_pos, non_zero_query_point_neg)]
                pos_neg_indicator = [torch.cat([torch.ones(x.shape[0], device=x.device), -torch.ones(
                    y.shape[0], device=y.device)]) for x, y in zip(non_zero_query_point_pos, non_zero_query_point_neg)]
                ...
                spatial_tokens = point_sample(src_mask_features.permute(2, 3, 0, 1), non_zero_query_point.flip(
                    dims=(2,)).type(src_mask_features.dtype), align_corners=True).permute(2, 0, 1)
                spatial_tokens[pos_neg_indicator ==
                               1] += self.pn_indicator.weight[0:1]
                spatial_tokens[pos_neg_indicator == -
                               1] += self.pn_indicator.weight[1:2]
        ...
        # for EVA02 checkpointing, when not in visual prompt mode, make a fake loss to ensure all parameters participate in loss calculation
        if hasattr(self, 'sot_fuser') and not self.find_unused_params:
            fake_fuser_loss = self.sot_fuser([torch.zeros(1, 256, 1, 1).to(zero_loss), torch.zeros(
                1, 256, 1, 1).to(zero_loss), torch.zeros(1, 256, 1, 1).to(zero_loss)])
        ...
        if task in ['vis', 'ovis', 'ytvis19', 'ytvis21', 'uvo_video', 'burst', 'rvos', 'coco_clip', 'obj365_clip', 'sa1b_clip',
            ...
            outputs = self.predictor(
                multi_scale_features, mask_features, extra=extra, task=task, masks=None, targets=targets)
            track_loss = self.get_tracking_contrastive_lossv3(
                outputs[0], targets, task)
    ...
    def video_visualP(self, images, prompts, task, targets=None, batch_name_list=None, is_train=True):
        ...
        # setp 1,2,3
        key_images = [images.tensor[kid].unsqueeze(
            0) for kid in key_prompt_frame]  # bz*[1,3,H,W]
        key_promptmasks = [m.unsqueeze(0)
                           for m in prompts['spatial']]  # bz*[1,1,H,W]
        ...
        ref_feats, ref_masks = self.get_template(
            key_images, key_promptmasks, prompt_mode)
        ref_feats = ref_feats[:, None, :, :].repeat(
            1, nframe, 1, 1).flatten(0, 1)  # [bz,L,c]
        ref_masks = ref_masks[:, None, :].repeat(
            1, nframe, 1).flatten(0, 1)  # [bz,L]

        early_fusion = {"hidden": ref_feats, "masks": ref_masks}
        if early_semantic is not None:
            early_fusion["hidden"] = torch.cat(
                [early_fusion["hidden"], early_semantic["hidden"]], dim=1)
            early_fusion["masks"] = torch.cat(
                [early_fusion["masks"], early_semantic["masks"]], dim=1)

        #######

        # bz = len(images)//2
        mask_features, _, multi_scale_features, zero_loss = self.pixel_decoder.forward_features(
            features, masks=None, early_fusion=early_fusion)
        ...
        prompt_multi_scale_features = [
            featurei[key_prompt_frame, :, :, :] for featurei in multi_scale_features]
        ...
        if 'spatial' in prompts:
            ...
            divisor = torch.tensor(
                [h, w], device=prompt_mask_features.device)[None,]
            # Get mean pos spatial query
            non_zero_pos_point = [rand_sample((m.nonzero()[:, 1:]/divisor).t(
            ), self.max_spatial_len[-1]).t() for m in extra['spatial_query_pos_mask']]
            non_zero_pos_point = nn.utils.rnn.pad_sequence(
            ...
            non_zero_pos_mask = (non_zero_pos_point.sum(dim=-1) < 0)
            spatial_query_pos = point_sample(prompt_mask_features, non_zero_pos_point.flip(
                dims=(2,)).type(prompt_mask_features.dtype), align_corners=True)  # [(N, C, P)
            spatial_query_pos = torch.stack([x[m].mean(dim=0, keepdim=True) for x, m in zip(
                spatial_query_pos.transpose(1, 2), ~non_zero_pos_mask)]).transpose(0, 1).nan_to_num()  # [1,bz,C]
            # Get mean neg spatial query
            non_zero_neg_point = [rand_sample((m.nonzero()[:, 1:]/divisor).t(
            ...
            non_zero_neg_point = nn.utils.rnn.pad_sequence(
                non_zero_neg_point, padding_value=-1).permute(1, 0, 2)
            non_zero_neg_mask = (non_zero_neg_point.sum(dim=-1) < 0)
            spatial_query_neg = point_sample(prompt_mask_features, non_zero_neg_point.flip(
                dims=(2,)).type(prompt_mask_features.dtype), align_corners=True)
            spatial_query_neg = torch.stack([x[m].mean(dim=0, keepdim=True) for x, m in zip(
                spatial_query_neg.transpose(1, 2), ~non_zero_neg_mask)]).transpose(0, 1).nan_to_num()
            ...
            for i in range(len(prompt_multi_scale_features)):
                ...
                # src_mask_features = multi_scale_features[i].view(h,w,bs,dc)
                src_mask_features = prompt_multi_scale_features[i].permute(
                    2, 3, 0, 1)
                src_mask_features = src_mask_features @ self.mask_sptial_embed[i]
                ...
                spatial_tokens[pos_neg_indicator == -
                               1] += self.pn_indicator.weight[1:2]
                spatial_tokens = spatial_tokens[:, :, None, :].repeat(
                    1, 1, nframe, 1).flatten(1, 2)
                non_zero_query_mask = non_zero_query_mask[:, None, :].repeat(
                    1, nframe, 1).flatten(0, 1)
                src_spatial_queries += [spatial_tokens]
                src_spatial_maskings += [non_zero_query_mask]
        ...
        outputs = self.predictor(multi_scale_features, mask_features,
                                 extra=extra, task=task, masks=None, targets=targets)
        fake_track_loss = (outputs[0]['pred_track_embed']*0).sum()
        return outputs, fake_track_loss, dist_loss + params_zero_loss
    ...
    def get_template(self, imgs, pad_masks, prompt_mode='scribble'):
        ...
        for image_i, mask_i in zip(imgs, pad_masks):
            ...
            im_crop = F.interpolate(
                im_crop, (256, 256), mode='bilinear', align_corners=False)
            croped_img_with_mask.append(im_crop)
        croped_img_with_mask = torch.cat(
            croped_img_with_mask, dim=0)  # [bz,3,256,256]
        with torch.no_grad():
